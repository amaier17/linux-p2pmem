 ...
 */
#define PCI_P2PDMA_ACS_FLAGS (PCI_ACS_RR | PCI_ACS_CR)

static int pci_p2pdma_disable_acs(struct pci_dev *pdev)
{
	int pos;
	u16 ctrl;
	...
	downstream->p2p_old_acs_flags = ctrl & PCI_P2PDMA_ACS_FLAGS;

	if (downstream->p2p_old_acs_flags)
		dev_info(&pdev->dev, "disabling peer-to-peer ACS flags: %x\n", ctrl);

	ctrl &= ~PCI_P2PDMA_ACS_FLAGS;
...
static int pci_p2pdma_reset_acs(struct pci_dev *pdev)
{
	int pos;
	u16 ctrl;
	struct pci_dev *downstream;
	...
	if (downstream->p2p_old_acs_flags)
		dev_info(&pdev->dev, "resetting peer-to-peer ACS flags: %x\n", ctrl);
...
int pci_p2pdma_add_client(struct list_head *head, struct device *dev)
{
	...
	if (item && item->p2pdma) {
		...
		ret = pci_p2pdma_disable_acs(client);
		if (!ret)
			goto put_client;
	}
...
{
	if (item->p2pdma)
		pci_p2pdma_reset_acs(item->client);
...
void pci_p2pdma_remove_client(struct list_head *head, struct device *dev)
{
	...
	list_for_each_entry_safe(pos, tmp, head, list) {
		...
		if (!p2pdma)
			p2pdma = pci_dev_get(pos->p2pdma);

		pci_p2pdma_client_free(pos);
	}

	if (p2pdma && list_empty(head))
		pci_p2pdma_reset_acs(p2pdma);
...
void pci_p2pdma_client_list_free(struct list_head *head)
{
	...
	pos = list_first_entry_or_null(head, struct pci_p2pdma_client, list);
	if (pos && pos->p2pdma)
		pci_p2pdma_reset_acs(pos->p2pdma);
...
static int bind_clients(struct pci_dev *p2pdma, struct list_head *clients)
{
	int ret;
	struct pci_p2pdma_client *pos, *unwind_pos;

	ret = pci_p2pdma_disable_acs(p2pdma);
	if (ret)
		return ret;

	list_for_each_entry(pos, clients, list) {
		ret = pci_p2pdma_disable_acs(pos->client);
		if (ret)
			goto unwind;

		pos->p2pdma = pci_dev_get(p2pdma);
	}
...
unwind:
	list_for_each_entry(unwind_pos, clients, list) {
		...
		pci_p2pdma_reset_acs(unwind_pos->client);
		pci_dev_put(unwind_pos->p2pdma);
		unwind_pos->p2pdma = NULL;
	}
 ...
 */
int pci_p2pdma_map_sg(struct scatterlist *sg, int nents)
{
	struct dev_pagemap *pgmap;
 ...
 * @nents:	elements in the scatterlist
 */
void pci_p2pdma_unmap_sg(struct scatterlist *sg, int nents)
{
	struct scatterlist *s;
	int i;
